...
def get_url(headers):
    ...
    pic_id=re.findall('class="ranking-image-item"><a href="/artworks/(.*?)"class=',resp.text,re.S)
    return thumbnail,pic_id
thumbnail,pic_id=get_url(headers)

win=Tk()
win.title('Pixiv')
# win.geometry('1100x1200')
...
for i in range(len(thumbnail)):
    ...
    image=Button(text,image=img_list[i],command = lambda x=i: download_picture(pic_id[x]))
